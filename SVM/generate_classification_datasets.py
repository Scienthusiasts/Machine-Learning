...
class classification():
    ...
    def BinarySample2D(self, point_num=500):
        ...
        x0_y = self.randSample(mu=-1, scale=0.3, num=point_num)
        x0 = np.concatenate((x0_x,x0_y),1)
        y0 = -np.ones(point_num)

        x1_x = self.randSample(mu=3, scale=1.2, num=point_num)
        x1_y = self.randSample(mu=-1, scale=0.3, num=point_num)
