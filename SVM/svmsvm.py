...
def testRbf(dataArr,  labelArr, testDataArr,testLabelArr, param, k1=1.3):
    # 加载训练集
    # dataArr, testDataArr, labelArr, testLabelArr = loadCSVfile2()
    # 根据训练集计算b, alphas
    b, alphas = smoP(dataArr, labelArr, 600, 0.0001, 100, (param, k1))
    print(b)
    ...
    for i in range(m):
        # 计算各个点的核
        kernelEval = kernelTrans(sVs, datMat[i, :], (param, k1))
    ...
    return (float(errorCount) / m), b, alphas
...
if __name__ == '__main__':
    ...
    errornum, b, α= testRbf(X,y,X,y,'lin')
    visualModel2D(X, y, b, α, 0)
